---
title: Introdução a Análise de Dados em R
author: Analytique Consultoria
date: Janeiro, 2019
css: custom.css
output:
...
acidentes_transito_2016 <- read.csv("dados/acidentes-transito-2016.csv", sep=";", stringsAsFactors=FALSE)
```
...
Agora iremos baixar outra base de dados, o banco "2018 EPI Country Snapshot" disponível no projeto [Enviromental Performance Index](https://epi.envirocenter.yale.edu/epi-downloads). Entre no repositório, baixe o banco de dados e salve-o na sua pasta de trabalho e importe-o para o R. Iremos usar este banco para executar algumas estatísticas descritivas:


```{r, e}
library(readr)
EPIdata <- read_csv("dados/epi2018countrysnapshotv01.csv")
```


```{r}
# calcular media
mean(EPIdata$EPI2018Score)

# mediana
median(EPIdata$EPI2018Score)

# desvio-padrao
sd(EPIdata$EPI2018Score)

# valor minimo e maximo
min(EPIdata$EPI2018Score)
max(EPIdata$EPI2018Score)

# funcao summary
summary(EPIdata$EPI2018Score)
```

### Selecionando casos máximos e mínimos

Com base no que aprendemos sobre manipulação de banco de dados é possível deduzir como construir um código que crie um banco de dados com os casos de valor máximo de uma variável e outro com os valores mínimos. Tente desenvolver com base na variável "EPI2018Score".

```{r, echo = FALSE, results = 'hide', message = FALSE}
EPIdata_max <- EPIdata[EPIdata$EPI2018Score == max(EPIdata$EPI2018Score),]
EPIdata_min <- EPIdata[EPIdata$EPI2018Score == min(EPIdata$EPI2018Score),]
```
...
ggplot(data = EPIdata, aes(x = EPI2018Score))+
  geom_histogram()
...
# executar boxplot
ggplot(EPIdata)+
  geom_boxplot(aes(y = EPIdata$EPI2018Score, x = 1))
